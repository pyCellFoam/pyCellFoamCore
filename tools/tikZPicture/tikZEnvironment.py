...
#==============================================================================
    __slots__ = ('__name',
                 ...
                 '__tikZCircledArrows',
                 '__logger',
                 '__tikZNames','__tikZNamesChanged',
                 '__tikZPrefix',
...
#==============================================================================
    ...
    additionaTikZCodePre = property(__getAdditionaTikZCodePre,__setAdditionaTikZCodePre)
    ...
    def __setAdditionaTikZCodePost(self,a): 
        if type(a) is str:
            ...
            self.logger.error('Additional TikZ code must be a string')
    additionaTikZCodePost = property(__getAdditionaTikZCodePost,__setAdditionaTikZCodePost)
...
#-------------------------------------------------------------------------
    def __calcTikZNames(self):
        ...
        '''
        self.__tikZNames = [x.name for x in self.__tikZCoordinates+self.__tikZNodes]
...
#-------------------------------------------------------------------------        
    def addTikZCoordinate(self,name,coordinates,*args,**kwargs):
        ...
        if name in self.tikZNames:
            error = True
            self.logger.error('Cannot add TikZCoordinate, because name "{}" is already in use'.format(name))
            
        try:
            if not coordinates.shape == (self.dim,):
                error = True
                self.logger.error('Wrong dimension: coordinates must be a {}-dimensional vector, but its dimension is {}'.format(self.dim,coordinates.shape))
        except:
            self.logger.error('Could not check dimensions of coordinates "{}"'.format(coordinates))
            error = True
            
        if not error:
            self.logger.info('Adding TikZCoordinate {}'.format(name))
            tikZCoordinate = TikZCoordinate(name,coordinates,*args,tikZPicture=self,**kwargs)
...
#-------------------------------------------------------------------------
    def addTikZNode(self,name,coordinates,*args,**kwargs):
        ...
        if name in self.tikZNames:
            error = True
            self.logger.error('Cannot add TikZNode, because name "{}" is already in use'.format(name))
        ...
        if not error:
            ...
            tikZNode = TikZNode(name,coordinates,*args,tikZPicture=self,**kwargs)
    ...
    def addTikZPolygon(self,coordinates,*args,**kwargs):
        ...
        for c in coordinates:
            if c not in self.__tikZCoordinates+self.__tikZNodes:
                error = True
                self.logger.error('{} is not a coordinate in this TikZPicture'.format(c))
        
        if error:
            self.logger.error('Cannot create TikZPolygon from coordinates {}'.format([c.name for c in coordinates]))
        else:
            self.__tikZPolygons.append(TikZPolygon(coordinates,*args,**kwargs,tikZEnvironment=self))
            self.logger.info('Adding TikZPolygon with coordinates {}'.format([c.name for c in coordinates]))
...
#-------------------------------------------------------------------------
    def addTikZLine(self,start,end,*args,**kwargs):
        ...
        if error:
            self.logger.error('Cannot create TikZPolygon from coordinates {}'.format([c.name for c in [start,end]]))
        else:
            self.__tikZLines.append(TikZLine(start,end,*args,**kwargs,tikZEnvironment=self))
            self.logger.info('Adding TikZLine with coordinates {}'.format([c.name for c in [start,end]]))
...
#-------------------------------------------------------------------------            
    def addTikZCircledArrow(self,center,*args,**kwargs):
        ...
        '''
        circledArrow = TikZCircledArrow(center,*args,tikZEnvironment=self,**kwargs)
        if center in self.__tikZCoordinates+self.__tikZNodes:
