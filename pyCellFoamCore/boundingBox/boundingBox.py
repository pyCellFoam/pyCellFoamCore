    ...
    def __init__(self,xRange,yRange,zRange,preset=None,xMinName=None,xMaxName=None,yMinName=None,yMaxName=None,zMinName=None,zMaxName=None,**kwargs):
        ...
        for r in [xRange,yRange,zRange]:
            if type(r) is list:
                if len(r) == 2:
                    if r[1] <= r[0]:
                        _log.error('The second value of the range must be larger than the first, but {} is smaller than {}'.format(r[1],r[0]))
                else:
                    error = True
                    _log.error('range must be a list with length 2 but has length {}'.format(len(r)))
            else:
                error = True
                _log.error('range must be a list with 2 entries, but {} is given'.format(r))
        ...
        if preset is None and all([n is None for n in limitNames]):
            _log.info('No preset and no limit names are given. Using standard preset "vtk"')
            preset = 'vtk'
        ...
        if not preset:
            if not all([type(n) is str for n in limitNames]):
                error = True
                _log.error('All names for limits must be strings')
                for n in limitNames:
                    if type(n) is str:
                        _log.info('{} is ok'.format(n))
                    else:
                        _log.error('{} is not ok'.format(n))



        elif preset in ['vtk','pyplot']:
            if not all([n is None for n in limitNames]):
                _log.warning('A preset and custom names for limits are given. The prest overwrites the given names.')
            ...
            error = True
            _log.error('Unknown preset "{}"'.format(preset))
        ...
        if error:
            ...
            _log.error('An error occured, bounding box is not defined correctly')
        else:
    ...
    def intersectLineWithBoundingBox(self,point1,point2):
        ...
        # and return edge and intersection point
            ...
            if len(edge) == 1 and np.allclose(intersections[0][1],intersections[1][1]):
                ...
                if len(edge) != 1:
                    _log.error('{} and {} do not share an edge, an error must have occured'.format(*intersections))
                if not np.allclose(intersections[0][1],intersections[1][1]):
                    _log.error('Got different intersection points {} and {} for line {} to {}'.format(intersections[0][1],intersections[1][1],point1,point2))
                return None
