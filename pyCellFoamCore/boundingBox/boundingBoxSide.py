...
#==============================================================================
    def __init__(self,edge1,edge2,edge3,edge4,normalVec,*args,rotationWXYZ=[0,1,0,0],translationXYZ=[0,0,0],**kwargs):
        ...
        if len(corners) != 4:
            self.logger.error('A bounding box face must have exactly four corners, but this face has {}'.format(len(corners)))
        ...
        if np.linalg.norm(normalVec) != 1:
            self.logger.warning('Normal vector was normalized, it had length {}'.format(np.linalg.norm(normalVec)))
        ...
        if not list((self.__normalVec == np.array([0,0,0]))).count(True) == 2:
            self.logger.error('Normal vector for a bounding box side must be parallel to a coordinate axis')
        
        
            
            
        self.logger.info('Created BoundingBoxSide')
...
#            cc.printYellow(p-ranges[:,1] < 1e-5)
            ...
            if np.alltrue(p-ranges[:,0] > -1e-5) and np.alltrue(p-ranges[:,1] < 1e-5):
                return p
            else:
                return None
