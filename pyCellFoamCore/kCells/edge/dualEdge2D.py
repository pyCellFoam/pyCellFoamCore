...
# =============================================================================
    def __init__(self,
        ...
        if edge.dualCell1D is None:
            myPrintError('Something went wrong with the calculation ' +
        ...
        if edge.category1 == 'border':
            if len(edge.faces) == 1:
                ...
                if normv > self.tolerance:
                    ...
                    myPrintDebug(v)
                    if v[2] < -0.5:
                        myPrintDebug('Edge was in the wrong direction, ' +
                                     'turning around')
                        self.startNode = n1
                    ...
                    if v[2] < -0.5:
                        myPrintError('Edge {} is still in the '.format(self) +
                                     'wrong direction')
                    else:
                    ...
                    myPrintError('Direction vector of ' +
                ...
                if len(faces) == 2:
                    ...
                    if nGeo is None:
                        myPrintError('{} should have'.format(edge) +
            ...
            if len(edge.faces) == 2:
                ...
                if v1[2] < -0.5 or v2[2] < -0.5:
                    myPrintDebug('Dual edge was in the wrong direction, ' +
                ...
                self.logger.error('An inner edge in a 2D complex needs ' +
                                  'two faces')

        else:
            myPrintError('Unknown category {}'.format(edge.category) +
    ...
    def calcHeatFlow(self, myPrintDebug=None, myPrintError=None):
        ...
        if len(self.simpleEdges) == 1:
            ...
            if se.startNode.category == 'inner' \
                ...
                myPrintError('Cannot find the correct primal face ' +
                             'for interpolation')
                return None
            entries = [entry, ]
            ...
            myPrintDebug('Evaluating {} for simple edge {}'
            ...
            return None
            myPrintError('{} simple edges in dual edge {} cannot be handled!'
