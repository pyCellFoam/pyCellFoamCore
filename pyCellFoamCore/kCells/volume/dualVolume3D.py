...
#==============================================================================
    def __init__(self,node,*args,myPrintDebug=None,myPrintError=None,myPrintInfo = None,**kwargs):  
        ...
        myPrintInfo('Creating dual volume of node {}'.format(node.infoText))
        unalignedFaces = []
        for e in node.edges:
        ...
        myPrintDebug('Faces to define volume: {}'.format(unalignedFaces))
...
#                f.setUp()
    ...
    #                cc.printRed('{}: first face points in the wrong direction'.format(self.infoText))
                ...
                while len(unalignedFaces) > 0 and count < maxCount:
                    ...
                    if not found:
                        myPrintError('Cannot add one of the faces {} to volume {}'.format(unalignedFaces,self.infoText))
                        count = maxCount
...
#            cc.printMagenta('DualVolume3D:',unalignedFaces)
            self.setUp()
            if self.faces:
                myPrintInfo('Created dual volume of node {}'.format(node.infoText))
                self.dualCell3D = node
                node.dualCell3D = self
            else:
                myPrintError('Failed to create dual volume of node {}'.format(node.infoText))
