...
#==============================================================================
    def __new__(cls,face,*args,volume=None,**kwargs):
        ...
        elif len(face.simpleFaces) == 1 and len([e for e in face.edges if e.category=='additionalBorder']) == 1:
            additionalBorderEdges = [e for e in face.edges if e.category=='additionalBorder']
    ...
    def __init__(self,face,*args,myPrintDebug=None,myPrintError=None,volume=None,**kwargs):
        '''
        ...
        myPrintDebug('Calculating 2D dual of {}'.format(face))
...
#                    self.zCoordinate = dn.zCoordinate
            if not len(face.geometricEdges) == 1:
                myPrintError('Face {} has two barycenters and should so have one geometric edge, but has{}'.format(face.infoText,len(face.geometricEdges)))
...
#                
            ...
            if not len(face.geometricNodes) == 1:
                myPrintError('Face {} has three barycenters and should so have one geometric node, but has {}'.format(face.infoText,len(face.geometricNodes)))
            
            
                
        else:
            myPrintError('Cannot calculate 2D dual of face {} that has more than one simple Face, it has {}'.format(face.infoText,len(face.simpleFaces)))
        ...
        self.logger.debug('Initialized DualNode2D')
