...
#==============================================================================    
    ...
    def __new__(cls,volume,*args,**kwargs):
        ...
        if volume.category1 == 'border' and len([f for f in volume.faces if f.category1 == 'additionalBorder']) == 1:
#            print('OK')
            additionalBorderFace = [f for f in volume.faces if f.category1 == 'additionalBorder'][0]
            
            if len(additionalBorderFace.simpleFaces) == 1:
#                print('1')
                return DualNode2D(additionalBorderFace.simpleFaces[0].belongsTo,volume=volume)
            elif len(additionalBorderFace.simpleFaces) == 2:
#                print('2')
                
                edges1 = [se.belongsTo for se in additionalBorderFace.simpleFaces[0].simpleEdges]
                edges2 = [se.belongsTo for se in additionalBorderFace.simpleFaces[1].simpleEdges]
                rimEdge = None
                for e in edges1:
                    me = -e
                    if me in edges2:
                ...
                if rimEdge:
                    return DualNode1D(rimEdge,face=additionalBorderFace,volume=volume)   
                else:
                    return super(DualNode3D, cls).__new__(cls) 
...
#                print('3')
                ...
                if geometricNode:
                    return DualNode0D(geometricNode,volume=volume,face=additionalBorderFace,edge=[e for e in geometricNode.edges if e.isGeometrical])
                else:
                    return super(DualNode3D, cls).__new__(cls)
            else:
#                print('Standard')
...
#    INITIALIZATION
#==============================================================================
    def __init__(self,volume,*args,myPrintDebug=None,myPrintError=None,**kwargs):
        '''
...
#            myPrintDebug('Total num of faces in volume {}: {}'.format(volume.infoText,len(volume.faces)))
#            for f in volume.faces:
#                if f.category1 == 'additionalBorder':
#                    if additionalBorderFace == None:
#                        additionalBorderFace = f
#                    else:
#                        myPrintError('Volume {} has more than one additional border face, this should not be!'.format(volume.infoText))
                        
                        
            additionalBorderFaces = [f for f in volume.faces if f.category1 == 'additionalBorder']
            ...
            if len(additionalBorderFaces) == 0:
                ...
                y = 0
                z = 0
                myPrintError('Border volume {} should have exactly one additional border face'.format(volume.infoText)) 
                
                
                
            elif len(additionalBorderFaces) == 1:
                myPrintDebug('Volume {} has the additional border face {} with {} simple faces'.format(volume.infoText,additionalBorderFace.infoText,len(additionalBorderFace.simpleFaces)))
                additionalBorderFace = additionalBorderFaces[0]

#    1 simple face --> border
#-------------------------------------------------------------------------            
                if len(additionalBorderFace.simpleFaces) == 1:
                    myPrintDebug('Border face {} has one simple face, putting dual node in the barycenter of the face'.format(additionalBorderFace.infoText))
...
#-------------------------------------------------------------------------                    
                    ...
                    myPrintDebug('Border face {} has two simple faces, putting dual node in the barycenter of the connecting edge'.format(additionalBorderFace.infoText))
                    edges1 = [se.belongsTo for se in additionalBorderFace.simpleFaces[0].simpleEdges]
                    edges2 = [se.belongsTo for se in additionalBorderFace.simpleFaces[1].simpleEdges]
                        ...
                        if me in edges2:
                            if rimEdge == None:
                                if e.isReverse:
                                 ...
                                 myPrintError('Face {} has more than one connecting edge, this should not be!'.format(additionalBorderFace.infoText))
...
#                    myPrintDebug = cc.printYellow
                    ...
                    geometricNode = None
                    myPrintDebug('Border face {} has three simple faces, putting dual node in connecting corner'.format(additionalBorderFace.infoText))
                    geometricNodes = additionalBorderFace.geometricNodes
                    geometricEdges = additionalBorderFace.geometricEdges
                    myPrintDebug('Face {} has {} additional border nodes and {} additional border edges'.format(additionalBorderFace,len(geometricNodes),len(geometricEdges)))
                    if len(geometricNodes) == 1:
                        myPrintDebug('Only one geometric node, this must be the corner')
                        geometricNode = geometricNodes[0]
                        ...
                        for n in geometricNodes:
                            ...
                            for e in geometricEdges:
                                if not (e.startNode == n or e.endNode == n):
                                    ok = False
                            if ok:
                                myPrintDebug('Geometric node {} connects to all geometric edges'.format(n))
                                possibleNodes.append(n)
                        if len(possibleNodes) == 1:
                            geometricNode = possibleNodes[0]
                            myPrintDebug('Found geometric node for corner')
                        else:
                            myPrintError('Cannot find geometric node in the corner of volume {}'.format(volume))
                            
#                    for e in additionalBorderFace.geometricEdges:
#                        if e.startNode.isGeometrical:
#                            if geometricNode == None:
#                                geometricNode = e.startNode
#                            elif geometricNode != e.startNode:
#                                myPrintError('Additional border face {} should have only one geometric node'.format(additionalBorderFace.infoText))
#                        if e.endNode.isGeometrical:
#                            if geometricNode == None:
#                                geometricNode = e.endNode
#                            elif geometricNode != e.endNode:
#                                myPrintError('Additional border face {} should have only one geometric node'.format(additionalBorderFace.infoText))
#                                
#                    myPrintDebug('Found corner node: {}'.format(geometricNode.infoText))
                    ...
                    z = 0
                    self.delete()
                    myPrintError('Border face {} must have one, two or three simple faces, but has {}'.format(additionalBorderFace.infoText,len(additionalBorderFace.simpleFaces)))
                    
            
            
            else:
                myPrintWarning('Border volume with more than one additional Border face')
                x = volume.barycenter[0]
                y = volume.barycenter[1]
                z = volume.barycenter[2]
            ...
            z = 0
            self.delete()
            self.logger.error('Unknown category {} of volume {}'.format(volume.category1,volume.infoText))
...
#-------------------------------------------------------------------------         
        ...
        cc.printBlue('-'*30)
        cc.printWhite('Dual:',dn100,'3D primal:',dn100.dualCell3D,'3D dual:',dn100.dualCell3D.dualCell3D)    
        
        cc.printBlue()
        cc.printBlue('Example 2')
        cc.printBlue('-'*30)
        try:
            cc.printWhite('Dual:',dn200,'3D primal:',dn200.dualCell3D,'3D dual:',dn200.dualCell3D.dualCell3D) 
        except:
            cc.printRed('Error')
        try:
            cc.printWhite('Dual:',dn200,'2D primal:',dn200.dualCell2D,'3D dual:',dn200.dualCell2D.dualCell2D) 
        except:
            cc.printRed('Error')
        ...
        cc.printBlue('-'*30)
        try:
            cc.printWhite('Dual:',dn300,'3D primal:',dn300.dualCell3D,'3D dual:',dn300.dualCell3D.dualCell3D) 
        except:
            cc.printRed('Error')
        try:
            cc.printWhite('Dual:',dn300,'2D primal:',dn300.dualCell2D,'2D dual:',dn300.dualCell2D.dualCell2D) 
        except:
            cc.printRed('Error')
        try:
            cc.printWhite('Dual:',dn300,'1D primal:',dn300.dualCell1D,'1D dual:',dn300.dualCell1D.dualCell1D) 
        except:
            cc.printRed('Error')        
        ...
        cc.printBlue('-'*30)
        try:
            cc.printWhite('Dual:',dn400,'3D primal:',dn400.dualCell3D,'3D dual:',dn400.dualCell3D.dualCell3D) 
        except:
            cc.printRed('Error')
        try:
            cc.printWhite('Dual:',dn400,'2D primal:',dn400.dualCell2D,'2D dual:',dn400.dualCell2D.dualCell2D) 
        except:
            cc.printRed('Error')
        try:
            cc.printWhite('Dual:',dn400,'1D primal:',dn400.dualCell1D,'1D dual:', [e.dualCell1D for e in dn400.dualCell1D]) 
        except:
            cc.printRed('Error')
            
        try:
            cc.printWhite('Dual:',dn400,'0D primal:',dn400.dualCell0D,'0D dual:',dn400.dualCell0D.dualCell0D) 
        except:
            cc.printRed('Error')
        ...
        cc.printBlue('-'*30)
        try:
            cc.printWhite('Dual:',dn500,'3D primal:',dn500.dualCell3D,'3D dual:',dn500.dualCell3D.dualCell3D) 
        except:
            cc.printRed('Error')
        try:
            cc.printWhite('Dual:',dn500,'2D primal:',dn500.dualCell2D,'2D dual:',dn500.dualCell2D.dualCell2D) 
        except:
            cc.printRed('Error')
        try:
            cc.printWhite('Dual:',dn500,'1D primal:',dn500.dualCell1D,'1D dual:', [e.dualCell1D for e in dn500.dualCell1D]) 
        except:
            cc.printRed('Error')
            
        try:
            cc.printWhite('Dual:',dn500,'0D primal:',dn500.dualCell0D,'0D dual:',dn500.dualCell0D.dualCell0D) 
        except:
            cc.printRed('Error')
