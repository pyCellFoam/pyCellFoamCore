...
#            cc.printMagenta('2 Simple edges: returning 0D dual node at geometric node')
            ...
            if BStart ^ BEnd:
#                cc.printMagenta('One additonal border node, returning 0D dual node at B node')
                if BStart:
    ...
    def __init__(self,edge,*args,myPrintDebug=None,myPrintError=None,face=None,volume=None,**kwargs):
        '''
        ...
        if len(edge.barycenter) == 1:
            ...
            myPrintError('Cannot calculate 1D dual of edge {} that has more than one simple edge, it has {}'.format(edge.infoText,len(edge.simpleEdges)))
            self.delete()
