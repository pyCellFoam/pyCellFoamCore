...
#==============================================================================
    ...
    def __getShowArrow(self): 
        if self.belongsTo:
            ...
            self.logger.warning('Simple Edge does not belong to a real cell, using fixed standard value for showArrow')
            return True
    ...
    def __getLength(self):
        return np.linalg.norm(self.endNode.coordinates - self.startNode.coordinates)
...
#==============================================================================
    ...
    def intersectWithBoundingBox(self,boundingBox):
        ...
        res = boundingBox.intersectLineWithBoundingBox(self.startNode.coordinates,self.endNode.coordinates)
...
#-------------------------------------------------------------------------    
    ...
    def plotEdge(self,ax,*args,showArrow=None,showLabel=None,color=None,dx=0,dy=0,dz=0,**kwargs):
        '''        
        ...
        if not isinstance(color,tc.TUMcolor):
            self.logger.error('Cannot plot {} with the wanted color {} - it must be an instance of TUMcolor()'.format(self,color))
        ...
        # plot 3D arrow
        if showArrow:
            a = Arrow3D(v1+0.4*d,v1+0.6*d, mutation_scale=20, lw=linewidth, arrowstyle="-|>", color=color.html,ls=linestyle)
            ax.add_artist(a)
...
#    Plot edge using VTK
#-------------------------------------------------------------------------                   
    def plotEdgeVtk(self,myVtk,showArrow=None,showLabel=None,arrowDiameter=1,arrowLength=1,color = None,**kwargs):
        '''
        ...
        myVtk.addLine(self.startNode.coordinates,self.endNode.coordinates,lineWidth=4,color=color.rgb01)
        
        if showLabel:
            myVtk.addTextAnnotationNumpy(self.barycenter,self.labelText)
        
        if showArrow:
            myVtk.addArrowCenterDirection(self.barycenter,self.directionVec,color=color.rgb01,length=arrowLength,diameter=arrowDiameter)
        
#-------------------------------------------------------------------------
#    Plot edge using TikZ
#-------------------------------------------------------------------------    
    def plotEdgeTikZ(self,tikzpicture,showLabel=None,color=None,shortLabel=True,showArrow=None,**kwargs):
        '''
        ...
        if not start:
            self.logger.info('{} has no tikZNode yet, adding it as a simple TikZCoordinate'.format(self.startNode))
            self.startNode.plotNodeTikZ(tikzpicture,showInPlot=False)
            start = self.startNode.getTikZNode(tikzpicture)
        if not end:
            self.logger.info('{} has no tikZNode yet, adding it as a simple TikZCoordinate'.format(self.endNode))
            self.endNode.plotNodeTikZ(tikzpicture,showInPlot=False)
            end = self.endNode.getTikZNode(tikzpicture)
        
        
        if not (start in tikzpicture.tikZNodes+tikzpicture.tikZCoordinates and end in tikzpicture.tikZNodes+tikzpicture.tikZCoordinates):
            self.logger.error('Must add nodes to tikZPicture "{}" first'.format(tikzpicture))
        else:
            tikzpicture.addTikZLine(start,end,lineOptions,intermediateText=labelText,intermediatePosition=self.tikZLabelPosition)
