...
#    INITIALIZATION
#==============================================================================
    def __init__(self,start,end,*args,num=None,label='e',geometricNodes = [],**kwargs):
        '''
...
#==============================================================================
    ...
    def __getAllNodes(self): return [self.startNode,*self.geometricNodes,self.endNode]
...
#-------------------------------------------------------------------------          
    ...
    def setUp(self):
        ...
        if self.checkIfDuplicates(allNodes):
            self.logger.error('Duplicate nodes in definition of {}: {}'.format(self,allNodes))
            ...
            if all(allNodes):
                ...
                if len(self.__geometricNodes) == 0:
                    self.__simpleEdges.append(SimpleEdge(start = self.__startNode,
                                                            end = self.__endNode,
                                                            belongsTo = self,
                    ...
                    if num > 25:
                        num = 0
                    for (n1,n2) in zip(self.__geometricNodes[:-1],self.__geometricNodes[1:]):
        #                print(n1.infoText,n2.infoText)
                        self.__simpleEdges.append(SimpleEdge(start = n1,
                                                                end = n2,
                                                                belongsTo = self,
                                                                labelSuffix = '('+an.alphaNum(num)+')'))
                        num +=1
                        if num > 25:
                            num = 0
                    self.__simpleEdges.append(SimpleEdge(start = self.__geometricNodes[-1],
                                                            end = self.__endNode,
                                                            belongsTo = self,
                                                            labelSuffix = '('+an.alphaNum(num)+')'))
            
            else:
                self.logger.debug('This edge is not completely defined, maybe its deleted')
...
#-------------------------------------------------------------------------           
    def addFace(self,face):
        ...
        if face in self.__faces:
            self.logger.error('Face {} already belongs to edge {}!'.format(face.infoText,self.infoText))
        else:
...
#-------------------------------------------------------------------------               
    def delFace(self,face):
        ...
        if face in self.__faces:
            ...
            self.logger.info('Removed simple face {} from simple edge {}'.format(face.infoText,self.infoText))
        else:
            self.logger.error('Cannot remove simple face {} from simple edge {}!'.format(face.infoText,self.infoText)) 
...
#-------------------------------------------------------------------------              
    def delete(self):
        if self.faces:
            self.logger.error('Cannot delete {} because it belongs to a face'.format(self))
        else:
...
#    from kCells import Node
    ...
    with MyLogging('Edge') as ml:
        ...
        cc.printWhite('start:',e1.startNode,'end:',e1.endNode,'n2 is connected to',n2.connectedNodes,'n4 is connected to',n4.connectedNodes)
        cc.printWhite()
        cc.printBlue('Change start node of',e2)
        print('Edges of n2:',n2.edges)
        e1.startNode = n10
        cc.printWhite('start:',e1.startNode,'end:',e1.endNode,'n2 is connected to',n2.connectedNodes,'n4 is connected to',n4.connectedNodes,'n10 is connected to:',n10.connectedNodes)
        cc.printWhite()
        e1.endNode = n14
