...
#    INITIALIZATION
#==============================================================================
    def __init__(self,node,*args,myPrintDebug=None,myPrintError=None,myPrintInfo = None,**kwargs):     
        '''
        ...
        if node.category1 == 'inner':
            ...
            myPrintDebug('Assuming 2D complex')
            myPrintDebug('Creating 2D dual of inner node {}'.format(node.infoText))
            dualEdges = [e.dualCell2D for e in node.edges]
            dualSortedEdges = self.__sortEdges(dualEdges,myPrintDebug,myPrintError)
            myPrintDebug(dualSortedEdges)
            self.edges = dualSortedEdges
            self.setUp()
            
            if np.linalg.norm(self.normalVec-np.array([0,0,1])) > self.tolerance:
                myPrintDebug('Face pointed in the wrong direction')
            ...
            myPrintDebug('Creating 2D dual of border node {}'.format(node.infoText))
            ...
            if len(primEdges) == 2 and len(primEdges[0].faces) == 1:
                ...
                if dualSortedEdges:
                    ...
                    self.logger.error('Cannot build face with the edges {}'.format(dualSortedEdges))
                
            else:
                self.category1 = 'additionalBorder'
                self.category2 = 'additionalBorder'
                
                myPrintInfo('Dual face of border node in 3D complex')        
                myPrintDebug('Primal (additional) border edges that belong to the node {}'.format(primEdges))
                ...
                for e in primEdges:
                    if e.category1 == 'additionalBorder':
                        if e.dualCell2D:
                            dualEdges.append(e.dualCell2D)
        #                cc.printGreen('dual edge {} of primal edge {}'.format(str(e.dualCell2D),e.infoText))
                    else:
                        if not e.dualCell2D:
                            DualEdge2D(e)
                        dualEdges.append(e.dualCell2D)
...
#                myPrintDebug
                ...
                if dualEdgesSorted:
                    myPrintDebug('Dual (additional) border edges that belong to the node {}'.format(dualEdgesSorted))
        ...
        #                e.setUp()
                    ...
                    myPrintDebug('Simple edges around the dual face: {}'.format(dualSimpleEdges))
                    
                    if node.dualCell0D == None:
                        DualNode0D(node)
                    ...
                    for sem in dualSimpleEdges[1:-1]:
                        ...
                        edgesForFaces.append([e1,em,e2])
                        myPrintDebug('Creating triangle with dual edge {}'.format([e1.infoText,sem.infoText,e2.infoText]))
                        myPrintDebug('Nodes that should define the simple face: {} {} {} {} {} {}'.format(e1.startNode,e1.endNode,sem.startNode,sem.endNode,e2.startNode,e2.endNode))
        ...
        if error:
            self.delete()
            myPrintError('An error occured during the creation process of dual face --> deleting')
        else:
...
#==============================================================================
    ...
    def __sortEdges(self,edges,myPrintDebug,myPrintError):
        ...
        if not all([isinstance(e,BaseEdge) for e in edges]):
            myPrintError('Need list of edges, but got {}'.format(edges))
            return False
...
if __name__ == "__main__":
    ...
    from kCells.node import DualNode3D,DualNode2D
...
#        ml.setStreamLevel(n1.logger,ml.debug)
#        de = DualEdge3D(f0)
        dualEdges = []
        for f in faces:
