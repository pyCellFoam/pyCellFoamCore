...
#==============================================================================   
    def __getCoordinates(self): 
        if self.__coordinates is None:
            if self.myReverse:
                ...
                self.logger.error('No reverse defined, cannot give coordinates')
    ...
    def __getArea(self):
        if self.__area is None:
            if self.myReverse:
                self.__area = [[a for a in reversed(self.myReverse.area[0])],self.myReverse.area[1]]
            else:
    ...
    def __getBarycenter(self): 
        if self.myReverse:
            ...
            self.logger.error('Reversed simple face does not belong to a simple face, cannot return barycenter')
    ...
    def __getSimpleEdges(self): 
        if self.myReverse:
            return [se.myReverse for se in reversed(self.myReverse.simpleEdges)]
        else:
            self.logger.error('Reversed simple face does not belong to a simple face, cannot return simpleEdges')
            return []
