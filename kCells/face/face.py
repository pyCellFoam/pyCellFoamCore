...
on the left and on the right side. The result can be seen in the second picture.
...
#==============================================================================
    ...
    def __setTriangulationmethod(self,t):
        if t in ['center','alternating']:
            ...
            self.logger.error('Unknown triangulation method {} for face {}'.format(t,self))
    triangulationMethod = property(__getTriangulationmethod,__setTriangulationmethod)
...
#                cc.printYellow(self.__rawEdges)
                if len(self.__rawEdges) == 1:
#                    cc.printGreen('Sorting edges')
                    self.__sortRawEdges()
#                    cc.printYellow(self.__rawEdges)
#                    
                else:
                    self.logger.error('Sorting of edges is only possible with one predefined closed circle of edges')
                
                
               
            self.logger.debug('{} simple faces are needed for face {}'.format(str(len(self.__rawEdges)),self))
            allEdges = []
            allNodes = []
            num = 0
            ...
            for subEdges in self.__rawEdges:
                ...
                for e in subEdges[1:]:
                    ...
                    found = False
                    # Go through all the already found simple edges, but in inversed direction
                    for seOld in reversed(simpleEdges):
                        if not found:
                            # Got through the simple edges of the new edge to be added
                            for seNew in e.simpleEdges:
                                self.logger.debug('Trying edge with start {} and end {}'.format(seNew.startNode,seNew.endNode))
                                # If the nodes are the same, then the first simple edge that should be added is found
                                if seNew.startNode == seOld.endNode:
                                    found = True
                                    self.logger.debug('node {} and {} are the same'.format(seNew.startNode,seOld.endNode))
                                else:
                                    self.logger.debug('node {} and {} are not the same'.format(seNew.startNode,seOld.endNode))
                                    self.logger.debug('seNew.endNode {}'.format(seNew.endNode))
                                    
                                # If the beginning was found (maybe in a previous iteration of this for loop), then 
                                # add all other simple edges of this new edges
                                if found:
                                    self.logger.debug('found')
                                    simpleEdges.append(seNew)
                                    
                        # If a connection between the last simple edge and the new full edge is not found,
                        # then there were to may simple edges from the last edge added to simpleEdges, therefore
                        # Remove the last one and try again
                        if not found:
                            self.logger.debug('removing')
                            simpleEdges.remove(seOld)
                            
                # The list of simple edges now looks like that: [? ? ok ok ok ok ? ? ?]
                if len(simpleEdges)>0:
                    nodes = [simpleEdges[0].startNode,]
                    for se in simpleEdges:
                        nodes.append(se.endNode)
                    self.logger.debug('Nodes before cutting first and last simple edges in subface {} of face {}: {}'.format(an.alphaNum(num),self.infoText,', '.join(n.infoText for n in nodes)))
#                        
#                        
#                        
                    # Start and end of this list still has to be adjusted
                    count = 0
                    while len(simpleEdges) > 0 and simpleEdges[-1].endNode != simpleEdges[0].startNode and count < 50:
                        count += 1
                        # Check if the last node occurs in the other nodes again and could maybe close the circle
                        if nodes[-1] in nodes[:-1]:
                            ...
                            if len(simpleEdges) > 0:
                                ...
                                nodes.pop()
#                            
                        # Check if the first node occurs in the other nodes again and could maybe close the circle
...
#                                    
#                                    
                    # The list of simple edges now should like that: [ok ok ok ok ok ok]
                    # Check that the cycled could really be closed and that it is not empty
                    if len(simpleEdges) > 0 and simpleEdges[-1].endNode == simpleEdges[0].startNode:
                        ...
                        if len(simpleEdges) == 0 or len(nodes) == 0:
                            self.logger.error('Removed all simple edges while trying to close the cycle of edges: {} in face {}'.format(', '.join([se.infoText for se in subEdges]),self.infoText))
                            self.delete()
#                            
#                            
#                            
                    ...
                    for e in subEdges:
#                            
                        # If the reversed edge is also part of the same face, this is a geometrical edge, otherwise it is a real edge
                        if e not in allEdges and not e.isGeometrical:
                            ...
                            if me in allEdges:
                                ...
                                try:
                                    ...
                                    print('Error, when setting {} to geometrical'.format(str(e)))
                            else:
                                allEdges.append(e)
#        
#        
                    for n in nodes:
                        if not n in allNodes:
                            allNodes.append(n)
...
#                            cc.printGreen('Original simpleEdges:',simpleEdges)
                        ...
                        if self.__triangulationMethod == 'alternating':
                            ...
                            se1 = simpleEdges[0]
                            se2 = simpleEdges[1]
                            ge2 = Edge(se2.endNode,se1.startNode,isGeometrical=True)
                            localSimpleEdges = [se1,se2,ge2.simpleEdges[0]]
                            
                            self.__simpleFaces.append(SimpleFace(localSimpleEdges,
                                                                 belongsTo = self,
                            ...
                            for i in range(numEdgesToDo-2):
                                ...
                                if fromEnd:
                                    ...
                                    edgesToDo.pop()
                                    ge2 = Edge(ge1.endNode,nextSimpleEdge.startNode,isGeometrical=True)
                                    localSimpleEdges = [ge1.simpleEdges[0],ge2.simpleEdges[0],nextSimpleEdge]
                                    ...
                                    edgesToDo.pop(0)
                                    ge2 = Edge(nextSimpleEdge.endNode,ge1.startNode,isGeometrical=True)
                                    localSimpleEdges = [nextSimpleEdge,ge2.simpleEdges[0],ge1.simpleEdges[0]]
                        ...
                        # -------------------------------                                
                            ...
                            nodesInSimpleFace = [se.startNode for se in simpleEdges]
                            newCoords = np.zeros(3)
                            for n in nodesInSimpleFace:
                                newCoords += n.coordinates
                            newCoords /= len(nodesInSimpleFace)
                            newNode = Node(newCoords[0],newCoords[1],newCoords[2],isGeometrical=True)
                            ...
                            ge2 = Edge(currentSimpleEdge.startNode,newNode,isGeometrical=True)
                            ge2Save = ge2
                            for (num,se) in enumerate(simpleEdges[:-1]):
                                ge1 = ge2
                                ge2 = Edge(se.endNode,newNode,isGeometrical=True)
                                self.__simpleFaces.append(SimpleFace([se,ge2.simpleEdges[0],-ge1.simpleEdges[0]],
                                                          belongsTo = self,
                                                          labelSuffix = '('+an.alphaNum(num)+')'))
                            ...
                            ge1 = ge2
                            ge2 = ge2Save
                            self.__simpleFaces.append(SimpleFace([simpleEdges[-1],ge2.simpleEdges[0],-ge1.simpleEdges[0]],
                                                      belongsTo = self,
                                                      labelSuffix = '('+an.alphaNum(num)+')'))
                            
#                                cc.printMagenta('New Node',newNode)
#                                newNode.printNode()
                        ...
                        self.__simpleFaces.append(SimpleFace(simpleEdges,
                                                             belongsTo = self,
                                                             labelSuffix = '('+an.alphaNum(num)+')'))
                        num += 1
                else:
                    self.logger.error('Subface of {} is not defined correctly. Edges {} do not define a closed circle'.format(self.infoText,subEdges))
...
#                    for e in group:
                    if e.isReverse:
                        e = -e
                    if not e in allEdges and not -e in allEdges and not e in self.__geometricEdges:
                        self.__geometricEdges.append(e)
                        
                        
            self.__geometricNodes = []
            for n in allNodes:
                if n.isGeometrical and not n in self.__geometricNodes:
            ...
            for e in self.__edges:
                for n in e.topologicNodes:
                    if not n in self.__topologicNodes:
...
#        ml.setStreamLevel(self.logger,ml.debug)
        ...
        tol = 1E-3
        edgesToBeRemoved = []
        self.logger.info('{}: Simplifying. Old raw edges: {}'.format(self.infoText,self.__rawEdges))
        for e in self.__geometricEdges:
            attachedSimpleFaces = []
            for sf in self.__simpleFaces:
                if any([se in sf.simpleEdges for se in e.simpleEdges]) or any([se.myReverse in sf.simpleEdges for se in e.simpleEdges]):
                    attachedSimpleFaces.append(sf)
            self.logger.debug('geometric edge {} is attached to {}'.format(e,attachedSimpleFaces))
            
            
            if len(attachedSimpleFaces)>1:
                normalVec0 = attachedSimpleFaces[0].normalVec
            
                if all([np.linalg.norm(normalVec0-sf.normalVec)<tol for sf in attachedSimpleFaces[1:]]):
                    if not e in edgesToBeRemoved:
                        edgesToBeRemoved.append(e)
                else:
                    self.logger.debug('Cannot be removed')
                    
            else:
                self.logger.error('{}: geometric edge {} should belong to a minimum of 2 simple faces, but only belongs to {}!'.format(self.infoText,e.infoText,len(attachedSimpleFaces)))
        ...
        for e in edgesToBeRemoved:
            ...
            if pack1 == pack2:
                ...
                if e in pack1 and -e in pack1:
                    pack1.remove(e)
                    pack1.remove(-e)
                    self.logger.debug('Edge {} and its reverse are in the same pack, it is now: {}'.format(e,pack1))
                    
                else:
                    self.logger.error('error, both packs are the same, but do not contain the removable edge and its reverse')
                ...
                for e1 in pack1:
                    if e == e1:
                        found = True
                    if not found and not e1 in pack12:
                        pack12.append(e1)
                found = False
                for e2 in pack2:
                    if found and not e2 in pack12:
                        pack12.append(e2)
                    if -e == e2:
                        found = True
                found = False
                for e2 in pack2:
                    if -e == e2:
                        found=True
                    if not found and not e2 in pack12:
                        pack12.append(e2)
                        
                found = False
                for e1 in pack1:
                    if found and not e1 in pack12:
                        pack12.append(e1)
                    if e1 == e:
                        found = True
                        
                self.logger.debug('{}: New pack: {}'.format(self.infoText,pack12))
...
#            self.data.rawEdges.remove(-e)
        ...
        self.logger.info('{}: Simplified simple faces, new raw edges: {}'.format(self.infoText,self.__rawEdges))
        self.updateGeometry()
...
#-------------------------------------------------------------------------    
    def interpolateLinear(self,myPrintDebug = None,myPrintError = None):
        ...
        if len(self.simpleFaces) != 1:
            myPrintError('Must have 1 simple face, but {} has {}'.format(self,len(self.simpleFaces)))
            return None
        else:
            myPrintDebug('Number of simple faces is ok')
            
        if len(self.edges) != 3:
            myPrintError('Must have 3 edges, but {} has {}'.format(self,len(self.edges)))
            return None
        else:
...
#-------------------------------------------------------------------------    
    ...
    def addVolume(self,v):
        if v in self.__volumes:
            self.logger.error('Volume {} already belongs to face {}'.format(v,self))
        else:
    ...
    def delVolume(self,v): 
        if v in self.__volumes:
            ...
            self.logger.error('Face {} is not part of volume {}'.format(self,v))
...
#-------------------------------------------------------------------------            
    ...
    def __sortRawEdges(self):
        ...
        if counter >= maxCounter:
            self.logger.error('Cannot find closed circle to define face {}'.format(self.infoText)) 
            return False
...
#-------------------------------------------------------------------------         
    ...
    def plotDoc(cls):
        ...
        f1000 = Face([[e1000,-e1006,e1005],[e1001,e1002,e1003,e1004,e1006]],triangulate=True,triangulationMethod='alternating')
        
        f1001 = Face([e1000,e1001,e1002,e1003,e1004,e1005],triangulate=True,triangulationMethod='alternating')
        f1002 = Face([e1000,e1001,e1002,e1003,e1004,e1005],triangulate=True,triangulationMethod='center')
...
#    from kCells import Edge
    ...
    #-------------------------------------------------------------------------            
            ...
            f501 = Face([[e501,e505,-e504],[e502,e506,-e505],[e503,e504,-e506]],num=5)
    ...
    #-------------------------------------------------------------------------            
            ...
            f601 = Face([[e603,e608,e607],[-e607,e608,e602],[e601,e605,-e604],[e602,-e609,-e605],[e609,-e608,-e606],[e609,e606,-e608],[e603,e604,-e609]],num=601)
...
#            f701.setUp()
    ...
    #-------------------------------------------------------------------------
            ...
            cc.printYellow('Geometric edges before simplification:',f801.geometricEdges)
            mf = -f801
            ...
            cc.printYellow('Geometric edges after simplification:',f801.geometricEdges)
    ...
    ##        e406 = Edge(n403,n402,406,geometricNodes=n405)
    ##        edges400.append(e406)
    #        e405.geometricNodes = n405
    ...
    #-------------------------------------------------------------------------     
            ...
            cc.printGreen('showNormalVec:',mf601.showNormalVec,f601.showNormalVec)
            mf601.showBarycenter = True
            cc.printGreen('showBarycenter:',mf601.showBarycenter,f601.showBarycenter)
...
#            
    ...
    #-------------------------------------------------------------------------  
            ...
            f1100 = Face([e1100,-e1101,e1103,e1104,e1102],sortEdges=True,triangulate=True)
...
#            for sf in f1002.simpleFaces:
        ...
        # TODO: Was passiert, wenn man in einer reversedFace Kanten austauscht???
